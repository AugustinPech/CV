  ...
  switch (htmlType) {
    ...
    default:
      if (values[keys.findIndex((item) => item == className)] != null) {
        newChild.innerHTML =
          values[keys.findIndex((item) => item == className)];
      }
  }
...
function modalContentDisplay(object, modalContent) {
  ...
  newChild.appendChild(document.createElement("div")).innerHTML =
    object.description;
  ...
  object.longDesc.forEach((item) => {
    let puceType = "";
    puceType = "li";
    newChild.lastChild.appendChild(document.createElement(puceType)).innerHTML =
      item;
  });
  newChild.appendChild(document.createElement("ul"));
  object.ressources.forEach((obj) => {
    ...
    obj.link.forEach((item) => {
      puceType = "li";
      newChild.lastChild.lastChild.lastChild.appendChild(
        document.createElement(puceType),
      ).innerHTML = '<a href="' + item + '">' + item + "</a>";
    });
  });
  return newChild;
}
...
function shadowOnHover(block) {
  ...
  newChild.className =
    "shadowOnHover d-flex flex-row justify-content-center align-items-center";
}
function cardElement(object, parentBlock) {
  ...
  divTop.className +=
    " topDiv d-flex flex-column justify-content-center h-50 align-items-center";
  let strStyle =
    "background-image: url('" +
    values[keys.findIndex((item) => item == "image")] +
